...
- Owned production harness: `kiana-runner::KianaHarness`, exposed as `kiana run`.
- TUI component library in `kiana-tui` (search/filter/layout/charts/macros and related tests). These modules are landed as a crate; they are not all wired into the product `kiana tui` loop.
- Product TUI (`kiana tui` / `kiana-screens`) searchable prompt history:
  - `/history` opens the persisted picker (`KIANA_HOME/tui-history.jsonl`)
  - `/` filters the picker; `Ctrl+R` opens search history
- Overlay and fuzzy-search infrastructure in `kiana-tui` for the experimental binary.
...
- Merged `codex/*` and `feature/ctrl-r-search-history` remote branches were deleted.
- Generated `graphify-out/` dumps and local Superpowers session notes are gitignored.
- Unpublished ACP/plugins/scripting/VS Code work is preserved on `wip/unlanded-extensions`, not in the master workspace.
...
- Local RC evidence now bundles non-accepted proof drafts generated by local
  preflight, so release handoff packages show source-control, product,
  entitlement, release operations, and current-platform platform-security
  progress without claiming acceptance.
- Added `scripts/source-control-proof-report.sh` for generating local RC
  source-control proof drafts and validating accepted pushed/tagged release
  proof files during full preflight.
