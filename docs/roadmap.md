...
| 2026-09-27 | `EQ-47` CLI routing source slice：`kiana eval` 新增 run/capture/compare/explain/list 严格参数合同与 `kiana.eval-cli.v1` 映射，迁移命令统一返回现有 ControlPlane route；旧 `run --suite` 保留显式 compatibility local adapter；补充 eval.explain/eval.list wire/event registry、CLI/source guard、CI workflow 与 evaluation-cli baseline；不运行本地测试且不等待 CI | 待本提交 |
