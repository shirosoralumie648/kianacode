...
| 下一步 | `OA-06` EventStore commit observer | `OA-05` ports/fakes 已固定；下一步只在 Committed 边界通知 audit/metric/trace/receipt 投影，重放不重复副作用并建立 cursor | pre-commit 事件不可见；`Replayed` 只关联原 receipt；CAS conflict/Unknown 保留 fail-closed |
