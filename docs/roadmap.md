...
| 2026-09-19 | `CI-11` audit/recovery projection partial：新增 CredentialRecoveryProjection 与 CI-only recovery evidence fixture，绑定 identity/assignment/config/credential/audit/redaction digests、generation/authority epoch、lease/blockers；unknown schema/stale epoch/revision/lease missing/refresh failure/result_unknown 默认暂停，只有 explicit re-admission 才可 resume，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-07` workflow queue claim partial：新增 WorkflowQueueClaimContract，统一 parent/item scope、budget、path-lock、dependency、parallel-limit、duplicate/expiry 与 owner claim 校验；仅建立 deny-first source contract，AUT-08 继续负责 durable queue、lease、heartbeat、fence、reclaim 与 dispatch，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-08` workflow queue store partial：新增 WorkflowQueueLease、WorkflowQueueStore 端口、ControlPlane dispatch guard 与 CI-only MemoryWorkflowQueueStore，覆盖竞争 claim、owner/fence heartbeat、单调 fence reclaim、in-flight/Unknown recovery；不把进程内 fixture 写成 durable/cross-process，scheduler、EventLog replay、physical recovery 仍未完成，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-09` DaemonHost workflow service partial：新增单一 bounded Tokio mpsc scheduler/worker service，统一 queue claim/heartbeat/effect/fence/reclaim/tick，shutdown 按已接收顺序 fence active leases 并报告 recovery；不新增模型或 Broker 执行循环，仍未证明 durable ack、ClockPort timer、跨进程 restart/worker death 与 trigger execution，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-10` interval cursor partial：新增纯 `plan_interval_due`，统一 Skip/FireOnce/有界 CatchUp 的 occurrence key、next cursor、backlog/skip 计数与 overflow deny，并由 durable workflow Tick 复用；未宣称 ClockPort、durable cursor、timer/restart/外部 trigger proof，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-11` signed event ingress partial：新增 bounded ingress/source-policy/occurrence contracts 与 daemon HMAC verifier，强制 source/project/key/event allowlist、clock skew、payload field filter、dedupe 与 event-ref Fire material；payload 不进入 capability request，EventLog/HTTP/durable dedupe/live credential 仍未完成，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-12` trigger concurrency partial：DurableTrigger 新增 pending/fired occurrence digest maps（兼容旧 snapshot），planner 显式实现 Reject/Queue/Coalesce/Replace、128 pending bound 与 digest conflict deny，Replace 先标记 CancelRequested 再保留 successor；stop/reconcile/durable CAS 仍未完成，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
| 2026-09-19 | `AUT-13` Advance planner partial：新增 stable ready-node view、FanOut 32-branch bound、FanIn output digest/lexical aggregation、SubWorkflow fixed version/owner/depth/remaining-budget checks 与 child terminal replay fold；不执行 child/capability，durable reservation/worker/scope/lease/restart recovery 仍未完成，roadmap 行保持 ⏳；不运行本地测试，格式与 workspace 静态编译通过，CI 已触发但未等待 | 待本提交 |
