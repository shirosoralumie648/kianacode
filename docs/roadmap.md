...
| 2026-09-23 | `P4-J7-23` 单层重试/绝对时限/取消：Runner 作为唯一 retry driver，按 typed pre-send/HTTP rejection 和 side-effect/delta 分类；SDK 隐式重试禁用由相邻 P4-J7-18 config 保证；Retry-After 秒/日期、指数退避+jitter 与 admission/attempt/backoff 均受同一 deadline 约束，取消传到准入、attempt 和 backoff；新增 provider/runner deny 与成功 fixtures、跨 crate source guard、GitHub Actions workflow 和 baseline；不运行本地测试，定向 rustfmt/diff 通过；本地 `cargo check` 被缺失 memory_workbench 和既有 domain 编译错误阻断，CI fmt 同样未到 fixtures，状态保持 🔄 | 待本提交 |
