...
| 2026-09-23 | `P4-J7-18` Ollama NDJSON：增量文本、done/content fail-closed、稳定 synthetic tool ID/结果续接、profile load timeout 和 load/generation timing 投影；新增 provider fixtures、core source guard、GitHub Actions workflow 与 baseline；runs 35832638463/35832756821 因缺失 `memory_workbench.rs` 在 fmt 失败；CM-36 后 run 35846594919 fmt 通过但首个 provider test 编译 `kiana-domain` 时因 24 个新暴露的旧模块错误退出 101，后续 fixture 跳过；保持 🔄，本地不运行测试 | 待本提交 |
