...
| 当前 81 | `CO-06` 不可变工件、Evidence 与 Criterion 引用合同 | 新增 typed ArtifactId/EvidenceId/CriterionId、ArtifactProvenance、ArtifactVersion/ArtifactRef/EvidenceRef/Criterion，固定 content hash/size/scope/provenance/version；ArtifactContentPort 只按 artifact_id+version 读 blob 并拒绝 missing/hash drift，core 复用 confined/atomic path 生成 CompanyProof artifact metadata；CompanyArtifact、CriteriaSnapshot 保留 optional typed refs 兼容旧 facts，criterion ID 不再由文本去重；新增 domain/ports/core fixtures、CI workflow 与 artifact baseline；不运行本地测试 | `feature_status=implemented`（domain/ports/core/protocol source + remote fixture wiring）、`proof_level=source`；本地只做格式与 workspace test-target 静态编译，GitHub Actions 已触发且未等待；typed blobs 尚无 durable cross-process store/CAS/recovery/retention，旧 CompanyState/BusinessState 尚未全面 upcast，Review/Delivery/Acceptance projector 与 freshness/availability 对账留待 CO-07+；下一步领取总 roadmap 中下一个无前置且未完成 step |
