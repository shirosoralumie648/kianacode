...
| 4 | **Harness：Agent 运行时** | 驱动一次 Agent 执行，调用模型、接收工具调用、等待工具结果、继续下一轮、压缩上下文、限制步数并支持受控继续 | [H02 身份基线](roadmap/harness-identity-baseline.md)、[KianaHarness](../kiana-runner/src/harness.rs)、[Runner 协议](../kiana-runner-protocol/src/lib.rs)、[上下文压缩](../kiana-runner/src/compact.rs)；Harness 只负责规范的单次运行循环，不负责绕过 ControlPlane 执行工具 |
