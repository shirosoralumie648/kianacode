...
| 5 | **Provider：模型接入** | 模型路由、请求编译、鉴权头、流式响应、工具调用解析、超时、重试分类、能力声明和模型用量回传 | [P4-J7-05 严格响应基线](roadmap/provider-strict-response-baseline.md)、[Provider crate](../kiana-provider/src/lib.rs)、[Daemon 模型适配](../kiana-daemon/src/model_client.rs)；Anthropic、OpenAI 兼容、Ollama 等协议属于模型接入，不等于外部业务集成，也不拥有用户或项目身份 |
