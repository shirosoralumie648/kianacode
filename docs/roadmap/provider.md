...
- 当前 source slice 与 CI-only 证据见 [`p4-j7-15-anthropic-baseline.md`](p4-j7-15-anthropic-baseline.md)。
...
- **CI 结果**：GitHub Actions runs `35832638463`（PR）和 `35832756821`（合并后）均在 `cargo fmt --all --check` 因 `kiana-domain/src/memory_workbench.rs` 缺失而失败，测试步骤跳过。CM-36 已补齐该文件；当前提交把它加入 workflow path filter，触发新的 GitHub CI rerun，结果仍 pending/unobserved。
...
- **退出 / 证据**：独立进程 crash/reopen 证据才支持 durable；UI transcript 不作恢复来源，partial response 不伪装完整 assistant turn。
  ...
  当前 source slice 与 CI-only 验收见 [`p4-j7-27-provider-recovery-baseline.md`](p4-j7-27-provider-recovery-baseline.md)。
  `ProviderResumeBinding`、提交事实 history projection 和 in-flight model/capability 对账分类已登记；
  EventStore/artifact durable reopen、真实 provider continuation 和 live/physical effect evidence 仍保留在后续步骤。
