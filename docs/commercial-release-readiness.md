...
- App-server conversations, settings, secrets, sandbox, and git-status subresponses now have pinned JSON schemas and release/package gates beside the top-level `/app` contract schema.
...
- Bring live terminal walkthroughs for TUI permission, diff, onboarding, resume, history, settings readiness, app-server client flows, and context-search workflows to reference-level usability; full commercial preflight now requires `scripts/product-acceptance-report.sh full`, but the accepted target-customer signoff file remains external.
