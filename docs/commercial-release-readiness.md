...
- Finish provider ecosystem parity beyond Anthropic/fake/OpenAI-compatible/Ollama paths, including tool-capable Ollama operation and running opt-in live provider smoke reports against production-like credentials/daemons; OpenAI-compatible tool calls are covered by local mock gates, and provider credential readiness is visible through `auth status`, but live provider operation is not proven by default gates.
