...
- Bring live terminal walkthroughs for TUI permission, diff, onboarding, resume, history, settings readiness, and app-server client flows to reference-level usability; full commercial preflight now requires `scripts/product-acceptance-report.sh full`, but the accepted target-customer signoff file remains external.
