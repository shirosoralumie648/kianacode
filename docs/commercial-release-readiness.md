...
- `kiana context index --json`, `kiana context search <query> --json`, and `kiana context pack <query> --json` now provide deterministic local repository indexing, ranked lexical search, and bounded snippet packs as the first knowledge/RAG foundation without external services; release smoke and package lifecycle smoke validate the pinned `kiana.context-index.v1`, `kiana.context-search.v1`, and `kiana.context-pack.v1` contracts.
