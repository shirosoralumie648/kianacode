...
- `kiana plugin install/uninstall --scope user|project|local` now supports user, project `.kiana/plugins`, and local `.kiana/plugins.local` install roots while preserving the existing `KIANA_PLUGINS_DIR` override for user-scoped installs; release smoke covers project and local scoped install/uninstall paths.
