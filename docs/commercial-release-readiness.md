...
- `kiana plugin install/uninstall --scope user|project|local` now supports user, project `.kiana/plugins`, and local `.kiana/plugins.local` install roots while preserving the existing `KIANA_PLUGINS_DIR` override for user-scoped installs; `list`, `json`, `show`, `path`, and `validate` can inspect the same scoped roots, and release smoke covers project/local scoped install, read, validate, and uninstall paths.
