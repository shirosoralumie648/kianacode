...
- Direct-connect server now exposes a bearer-protected local app-server contract under `/app`, with read-only conversations, session event snapshots, settings, redacted secrets, sandbox, git-status, diff, checkpoint creation, checks dry-run/run, review dry-run/run, context-index, repo-map, context-search, and context-pack endpoints covered by `direct_connect_app_contract`.
...
- Bring live terminal walkthroughs for TUI permission, diff, onboarding, resume, history, settings readiness, app-server client flows, and context-search workflows to reference-level usability; app-server now exposes plugin status, bounded session event snapshots, context index, repo-map, context search, and context packs for clients, and full commercial preflight requires `scripts/product-acceptance-report.sh full`, but the accepted target-customer signoff file remains external.
