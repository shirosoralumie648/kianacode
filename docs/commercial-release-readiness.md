...
- Direct-connect server now exposes a bearer-protected local app-server contract under `/app`, with read-only conversations, session event snapshots, settings, redacted secrets, sandbox, git-status, diff, checks dry-run/run, review dry-run/run, context-index, context-search, and context-pack endpoints covered by `direct_connect_app_contract`.
