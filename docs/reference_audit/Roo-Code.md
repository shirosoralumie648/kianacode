...
- CLI 通过 `kiana diff --session-changes <session-id> --json` 暴露聚合视图，TUI `/diff` 已复用该合同渲染文件变化预览，后续 bridge 复用同一合同。
