...
| File change data | diff/checkpoint 命令已有；`Write`/`Edit`/`Delete` 工具现在都在 model-facing `tool_result.changed_files` 输出路径和操作类型，并通过 RuntimeEvent、stream-json、remote SDK adapter、bridge adapter、runtime/app-server schema 与 SDK 文档保留该元数据；`Delete` 还沿用 read-before-mutate、mtime、editable/read-only 安全校验；`kiana diff --session-changes <session-id> --json` 现在可从 session `events.jsonl` 聚合稳定的 file changes view，TUI `/diff` replay 也会把该 schema 渲染为文件变化预览而不是裸 JSON；bridge crate 现在通过 `bridge_session_changes_report()` 直接消费 runtime events 并生成同形 `kiana.diff.session_changes.v1` 聚合视图；`kiana checkpoint restore <checkpoint-dir-or-manifest> --json` 现在输出 `kiana.checkpoint.restore.v1`，并用 `changed_files` 记录 patch/untracked restore 的 operation、source、before_exists、after_exists 与 changed；`kiana doctor --json` 现在把 repo-map budget、file sets、file changes、checkpoint/undo/diff、isolated review/checks、repair loops 和 late-user-edit conflict 汇总为 `local-coding-workflow` readiness evidence | 本地 file-change 与 Phase 4 local-coding workflow audit 已补齐；剩余为外部 release proof | `kiana-tools/src/file_write.rs`, `kiana-tools/src/file_edit.rs`, `kiana-tools/src/file_delete.rs`, `kiana-types/src/runtime.rs`, `kiana-entrypoints/src/runner.rs`, `kiana-entrypoints/src/cli.rs`, `kiana-entrypoints/src/tui.rs`, `kiana-remote/src/sdk_message_adapter.rs`, `kiana-bridge/src/sdk_message_adapter.rs`, `kiana-commands/src/diff.rs`, `kiana-commands/src/checkpoint.rs`, `kiana-commands/src/doctor.rs` | `cargo test -p kiana-tools write_api_result_reports_changed_file_metadata`; `cargo test -p kiana-tools edit_api_result_reports_changed_file_metadata`; `cargo test -p kiana-tools delete_api_result_reports_changed_file_metadata`; `cargo test -p kiana-tools delete_requires_file_to_be_read_first`; `cargo test -p kiana-tools delete_rejects_files_not_listed_as_editable`; `cargo test -p kiana-commands diff_session_changes_json_aggregates_tool_result_changed_files`; `cargo test -p kiana-entrypoints tui_session_changes_diff_json_result_renders_file_changes_preview`; `cargo test -p kiana-bridge bridge_session_changes_report_aggregates_runtime_changed_files`; `cargo test -p kiana-commands checkpoint_restore_json_reports_before_after_file_changes`; `cargo test -p kiana-commands doctor_json_reports_complete_local_coding_workflow_audit`; `cargo test -p kiana-entrypoints runner_runtime_event_adapter_preserves_changed_files`; `cargo test -p kiana-entrypoints stream_json_partial_event_exposes_tool_result_changed_files`; `cargo test -p kiana-remote remote_sdk_adapter_emits_runtime_events_for_messages_tools_and_results`; `cargo test -p kiana-bridge bridge_sdk_adapter_preserves_tool_result_changed_files`; `cargo test -p kiana-types runtime_event_schema_covers_required_payloads`; `bash scripts/schema-contract-smoke.sh` | P1 |
...
- CLI 通过 `kiana diff --session-changes <session-id> --json` 暴露聚合视图，TUI `/diff` 已复用该合同渲染文件变化预览，bridge crate 也已复用同一合同生成 runtime event 聚合报告。

Acceptance Criteria:
- Edit/Write/Delete 工具均记录文件路径和操作类型，Delete 还要求文件先被读取且未发生 mtime 变化。
- checkpoint restore JSON 已可比较 restore 前后的 changed files。
- 没有 metadata 的 legacy session 仍可通过 git diff fallback。

Tests:
- `cargo test -p kiana-commands diff_session_changes_json_aggregates_tool_result_changed_files`
- `cargo test -p kiana-commands checkpoint_restore_json_reports_before_after_file_changes`
- `cargo test -p kiana-bridge bridge_session_changes_report_aggregates_runtime_changed_files`
- `cargo test -p kiana-entrypoints tui_session_changes_diff_json_result_renders_file_changes_preview`
