...
| 8 | Local code workflow and git/checkpoints | `reference/aider/aider/repomap.py`, `reference/aider/aider/coders/search_replace.py`, `reference/cline/docs/core-workflows/checkpoints.mdx`, `reference/Roo-Code/src/core/webview/checkpointRestoreHandler.ts` | `kiana-query/src/repo_map.rs`, `kiana-tools/src/file_write.rs`, `kiana-tools/src/file_edit.rs`, `kiana-types/src/runtime.rs`, `kiana-entrypoints/src/runner.rs`, `kiana-entrypoints/src/cli.rs`, `kiana-remote/src/sdk_message_adapter.rs`, `kiana-bridge/src/sdk_message_adapter.rs`, `kiana-commands/src/diff.rs`, `kiana-commands/src/checkpoint.rs`, `kiana-commands/src/review.rs` | Write/Edit file-change metadata 已进入 `tool_result.changed_files`、RuntimeEvent、stream-json、remote/bridge adapter 和 schema，`kiana diff --session-changes <session-id> --json` 已可从 session events 聚合 file changes view；Delete metadata、checkpoint restore 对比、repo map budget behavior 仍需更稳 | ToolResult 写 changed_files；diff 读取 session metadata 并聚合；checkpoint/review 读取 metadata 与 git fallback | P1 |
