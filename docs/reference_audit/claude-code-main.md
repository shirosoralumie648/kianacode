...
| Resource cache | plugin skill cache 已按 enabled roots 刷新；plugin markdown commands 已支持非交互 CLI 执行且 disabled plugin commands 已从 active surfaces 隐藏；disabled plugin MCP 已从 resolver 与 `kiana mcp list/get` 隐藏；MCP user/project/local scope 已写入并按信任边界读取 | hooks/agents/LSP 全资源 disable visibility 仍需补齐，MCP 后续新 surface 还需同步生命周期断言 | `kiana-skills/src`, `kiana-commands/src/plugin.rs`, `kiana-commands/src/plugin_commands.rs`, `kiana-entrypoints/src/cli.rs`, `kiana-tools/src/lsp_tool.rs`, `kiana-tools/src/mcp_tool.rs`, `kiana-commands/src/mcp.rs` | resource visibility matrix + `cargo test -p kiana-commands plugin_commands` + `cargo test -p kiana-entrypoints plugin_prompt_commands_run_from_non_interactive_cli` + `cargo test -p kiana-commands mcp` + `cargo test -p kiana-tools mcp` | P1 |
