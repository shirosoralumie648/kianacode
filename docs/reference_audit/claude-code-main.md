...
| Hook CLI | 已有 hooks add/remove/status，且 malformed `hooks.json` 会通过 CLI schema error 与运行时 blocking error 可见化 | timeout/error event 还需继续补齐更明确的 public RuntimeError 事件语义 | `kiana-commands/src/hooks.rs`, `kiana-query/src/stop_hooks.rs`, `kiana-types/src/hooks.rs` | malformed hook fixtures 已覆盖；继续补 timeout/runtime-error fixtures | P1 |
