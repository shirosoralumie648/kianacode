...
| Hook CLI | 已有 hooks add/remove/status，且 malformed `hooks.json` 会通过 CLI schema error、运行时 blocking error、结构化 RuntimeError hook event 可见化；timeout/invalid JSON 也有稳定错误码 | non-zero exit 可配置为 deny 或 warning 仍待补齐 | `kiana-commands/src/hooks.rs`, `kiana-query/src/stop_hooks.rs`, `kiana-types/src/hooks.rs` | malformed hook、invalid JSON、timeout fixtures 已覆盖；继续补 non-zero policy fixtures | P1 |
