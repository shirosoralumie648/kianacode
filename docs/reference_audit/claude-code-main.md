...
| Command markdown | 插件命令作为 markdown prompt/command 文件安装，并可通过 CLI 直接执行 | `reference/claude-code-main (2)/claude-code-main/plugins/commit-commands/commands/commit.md`, `reference/claude-code-main (2)/claude-code-main/plugins/pr-review-toolkit/commands/review-pr.md`, `reference/claude-code-main (2)/claude-code-main/plugins/plugin-dev/skills/command-development/SKILL.md` | command 文件带 frontmatter 和 prompt body；命令系统发现并执行 | `kiana-commands/src/plugin_commands.rs`, `kiana-commands/src/registry.rs`, `kiana-entrypoints/src/cli.rs` | 是 | 只借 command contract，不复制 prompt 文案 |
...
| Resource cache | plugin skill cache 已按 enabled roots 刷新；plugin markdown commands 已支持非交互 CLI 执行且 disabled plugin commands 已从 active surfaces 隐藏；disabled plugin hooks/agents/LSP/MCP 均已有 active resolver 或命令级 fixture；MCP user/project/local scope 已写入并按信任边界读取 | MCP 后续新 surface 仍需同步生命周期断言；完整资源矩阵可继续扩成单一组合 fixture | `kiana-skills/src`, `kiana-commands/src/plugin.rs`, `kiana-commands/src/plugin_commands.rs`, `kiana-entrypoints/src/cli.rs`, `kiana-tools/src/agent.rs`, `kiana-tools/src/lsp_tool.rs`, `kiana-tools/src/mcp_tool.rs`, `kiana-commands/src/mcp.rs` | `cargo test -p kiana-commands plugin_commands`; `cargo test -p kiana-entrypoints plugin_prompt_commands_run_from_non_interactive_cli`; `cargo test -p kiana-commands mcp`; `cargo test -p kiana-tools disabled_plugin`; `cargo test -p kiana-tools plugin_agent`; `cargo test -p kiana-tools plugin_lsp` | P1 |
| Hook CLI | 已有 hooks add/remove/status，且 malformed `hooks.json` 会通过 CLI schema error、运行时 blocking error、结构化 RuntimeError hook event 可见化；timeout/invalid JSON 也有稳定错误码；non-zero exit 默认 deny，`KIANA_HOOK_NONZERO_POLICY=warning` 可作为 summary warning 不阻塞继续执行 | 更完整的 public UI/transcript hook warning 呈现仍可继续加强 | `kiana-commands/src/hooks.rs`, `kiana-query/src/stop_hooks.rs`, `kiana-types/src/hooks.rs` | malformed hook、invalid JSON、timeout、non-zero deny/warning fixtures 已覆盖 | P1 |
...
Acceptance Criteria:
- malformed hook schema 有 CLI error 与 runtime blocking error。
- invalid JSON/timeout 有更明确 RuntimeError。
- timeout 有明确错误。
