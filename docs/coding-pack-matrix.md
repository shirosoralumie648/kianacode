...
| CODE-04 | Provider 显式降级 | Anthropic / OpenAI-compatible / Ollama：缺 tools 或 streaming 必须在 `kiana run --json` 里出现机器可读错误，不能空转成功 | 本仓 `ProviderError::UnsupportedCapability`；OpenHands provider settings；pi reasoning-token；aider `models.py` | 已绿（fake text-only / unsupported_tools）；不是 live，不是 streaming |
...
`PHASES.md` v0.4 打开条件「有一份签字的公开行为矩阵草稿」= **本文件**。v0.4.4 CODE-04 fake text-only / `unsupported_tools` 已绿。下一独立 Phase 仅当需要才是 v0.4.5 P1-READ，否则 v0.5。
