...
**落地状态（2026-09-09）**：命令行 live 证据块（`5ecc4a3`）之后，`kiana run` 的普通路径已默认流式——`--no-stream` 或 `--json` 关闭，`--stream` 仍可显式开启；`--stream` 与互斥模式（`--receipt` / `--cancel` / `--packet` 等）同时出现时仍返回 `stream_requires_run`。网页版在传输层一直走 SSE，模型侧由 `KIANA_STREAMING=auto` 决定，实际已默认开启；网页版自己的 live 证据块仍未取得（需要真实 key 直连重跑）。
