...
- Local app-server `/app` contract returns `kiana.app-server.contract.v1` and
  ...
  context-artifact-ingest, context-search, and context-pack endpoints.
...
- Local RC evidence also stages non-accepted local proof drafts from
  `target/source-control`, `target/product-acceptance`,
  `target/entitlement-proof`, `target/release-ops`, and the current-platform
  `target/platform-security` proof into `dist/proofs/local-rc/` when those
  drafts were generated by local preflight.
...
- Source-control approval is attached as `kiana.source-control-proof.v1`,
  ...
  and release-owner acceptance timestamp; use
  `scripts/source-control-proof-report.sh --local-rc` for a non-accepted local
  draft and `scripts/source-control-proof-report.sh full` to validate the
  accepted release-owner proof.
...
- App-server context endpoints return `kiana.context-index.v1`,
  `kiana.context-artifacts.v1`, `kiana.context-artifact-ingest.v1`,
  `kiana.repo-map.v1`, `kiana.context-search.v1`,
  `kiana.context-vector-search.v1`, and `kiana.context-pack.v1` from the
  active workspace so Web/IDE clients can use repository knowledge without
