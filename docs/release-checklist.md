...
- Local RC evidence also stages non-accepted local proof drafts from
  `target/product-acceptance`, `target/entitlement-proof`,
  `target/release-ops`, and the current-platform `target/platform-security`
  proof into `dist/proofs/local-rc/` when those drafts were generated by local
  preflight.
