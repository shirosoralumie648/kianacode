...
#### Implemented: prefix-proof EventLog/state recovery

- Runtime 已覆盖 `EventLog append 成功、state.json 更新前崩溃` 的合法 crash window。
- `resume_workflow_run` 在 writer lease 内验证完整 EventLog，再用 `state.last_event_seq` 对应历史前缀证明旧 state；只有 schema、身份、请求、配置、状态、节点、checkpoint、pending approvals 与时间戳全部一致时才自动重放。
- 恢复结果固定为 `ready | repaired | blocked`；所有可继续入口统一接受 `ready | repaired`，审计输出保留 `repaired` 状态。
- state 写回采用同目录临时文件、文件 `sync_all`、原子替换与平台持久化语义；Unix 同步父目录，Windows 使用 `MoveFileExW` write-through。
- 已有 runtime 与 command 回归覆盖：显式/无参数 continue 自动修复、validate/evidence 修复后继续、state 超前阻断、请求/节点/checkpoint/pending approvals/时间戳篡改阻断。
- 尚未宣称的边界：真实 Windows/macOS release runner 的断电/文件系统故障注入证明仍属于平台验收证据，不由 Linux 本地单测替代。
...
- 2026-06-23: Ran the Phase 1 default verification gate: `cargo fmt --all --check`, `cargo test -p kiana-types`, split `cli_session`/`cli_resume` tests because a combined cargo filter is invalid, and `bash scripts/release-smoke.sh`. The release smoke completed with workspace tests, release build, install smoke, and doctor smoke. Phase 1 remains open because the exit criteria still require compact/export/import event-tree parity and shared rendering adoption. (The original plan file under `docs/superpowers/` was deleted on 2026-08-22.)
...
- 2026-07-13: Replaced the earlier project-local trust persistence model with the fail-closed `kiana.project-trust.v2` user trust root. Missing decisions now resolve to `ProjectTrust::Unknown`; records live outside the project at `$KIANA_HOME/trust/projects/<project_id>.json` and bind both the canonical project root and SHA-256 project id. Project-local `.kiana/trust.json` is legacy ignored diagnostics only and can never authorize its own hooks, skills, agents, MCP config, plugins, or mutating tools. `kiana trust trust|untrust|reset|status|json|path`, `/app/trust/status`, stream JSON skill discovery, non-streaming/streaming runner contexts, resident workers, and the local MCP server now consume the same effective trust decision. Unknown and Untrusted both block project resources and mutating tools before allow/bypass policy, while user/plugin/read-only resources remain available. Focused coverage is in `cargo test -p kiana-types trust --locked --offline`, `cargo test -p kiana-commands trust --locked --offline`, `cargo test -p kiana-tools --lib --locked --offline`, `cargo test -p kiana-query project_trust_source_boundary --locked --offline`, `cargo test -p kiana-skills --lib --locked --offline`, `cargo test -p kiana-entrypoints trust --locked --offline`, `cargo test -p kiana-entrypoints stream_json_init --locked --offline`, and the serial `cargo test -p kiana-entrypoints --lib --locked --offline -- --test-threads=1`. Remaining Phase 2 risk is platform-specific sandbox depth and future policy surfaces, not project-local self-authorization.
...
- 2026-07-07: Promoted distribution review into the local RC evidence bundle. `scripts/distribution-review-report.sh` writes `kiana.app-server.distribution-review.v1` JSON under `dist/proofs/local-rc/distribution/`, `scripts/local-rc-evidence-report.sh` includes it in the RC proof list, and the direct-connect distribution review now discovers nested winget manifests while preserving BLOCKED channel state. Focused coverage is in `bash scripts/commercial-release-handoff-smoke.sh`, `bash scripts/schema-contract-smoke.sh`, `KIANA_PREFLIGHT_SKIP_COMPLIANCE=1 bash scripts/release-preflight.sh --local-rc`, and `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked`.
- 2026-07-07: Staged commercial blocker handoff files inside the local RC evidence bundle. `scripts/local-rc-evidence-report.sh` writes `kiana.commercial-release-blockers.v1` JSON plus `commercial-release-handoff.md` under `dist/proofs/local-rc/blockers/`, and local RC evidence now counts that blocker proof alongside source-control, distribution, product, entitlement, release-ops, and platform-security drafts. Focused coverage is in `bash scripts/commercial-release-handoff-smoke.sh` and `KIANA_PREFLIGHT_SKIP_COMPLIANCE=1 bash scripts/release-preflight.sh --local-rc`.
...
- 2026-07-07: Added managed plugin signature verification policy. Enterprise policy files can now set `plugins.requireSignatureVerification = true` with `plugins.signatureVerificationCommand`; `kiana plugin install` verifies marketplace signature metadata and `contentHash`, then runs the administrator command with plugin root, marketplace, content hash, and signature fields in environment variables before copying the plugin. Failed external verification denies install before any files are written. Focused coverage is in `cargo test -p kiana-commands plugin_install_runs_managed_signature_verification_command --locked`, `cargo test -p kiana-commands plugin_install_rejects_failed_managed_signature_verification_command --locked`, `bash scripts/schema-contract-smoke.sh`, and `bash scripts/release-smoke.sh`. Remaining Phase 5 work is reduced to deeper third-party product plugin contracts beyond provenance verification.
- 2026-07-08: Added the pinned plugin app manifest product contract. Plugin `app.json` files now validate against the local `kiana.plugin-app-manifest.v1` expectation for safe `id`, `title`, relative `entry`, and optional route metadata, `/app/plugins` carries the same schema shape in its response contract, and release/package gates include the manifest schema. Focused coverage is in `cargo test -p kiana-commands --locked plugin_validate_rejects_invalid_app_manifest_contract`, `cargo test -p kiana-entrypoints direct_connect_app_contract --locked`, and `bash scripts/schema-contract-smoke.sh`. Remaining Phase 5 work is reduced to external marketplace provenance operations and signed distribution policy in real deployment channels.
...
- 2026-07-08: Aligned the public doctor/reference capability status for product shell to `local_ready_external_required`. The local product-shell evidence now covers headless TUI smoke, direct-connect app contract surfaces, release proof visibility, and local RC product-acceptance evidence; the remaining Phase 7 commercial blocker is accepted target-customer walkthrough and live Web/IDE client signoff, not missing local release evidence. Focused coverage is in `cargo test -p kiana-commands doctor_json_reports_reference_capability_matrix --locked --offline` and `cargo test -p kiana-commands doctor_text_reports_reference_capability_matrix --locked --offline`.
...
- 2026-07-07: Added bearer-protected model selection writes to the direct-connect product shell. `/app/models/current` now advertises `model.current.write`, accepts POST updates for built-in model profiles, persists through the same `model` command path as the CLI, and returns `409 CONFLICT` instead of pretending success when an app-state or `ANTHROPIC_MODEL` runtime override controls the effective model. Focused coverage is in `cargo test -p kiana-entrypoints direct_connect_app_model_current_post_updates_config_when_no_runtime_override --locked` and `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked`.
- 2026-07-07: Exposed persisted prompt history through the direct-connect product shell. `/app/prompt-history` now advertises `prompt.history.read` and returns pinned `kiana.app-server.prompt-history.v1` entries from `KIANA_HOME/tui-history.jsonl`, preserving the TUI loader's invalid-line tolerance and newest-first dedupe while avoiding raw home path leakage. Focused coverage is in `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked` and `cargo test -p kiana-entrypoints direct_connect_app_contract_schemas_match_packaged_schema_files --locked`.
- 2026-07-07: Exposed command discovery through the direct-connect product shell. `/app/commands` now advertises `commands.read` and returns pinned `kiana.app-server.commands.v1` visible core/plugin slash commands with command type, route intent, enabled state, non-interactive support, and source metadata for Web/IDE command palettes without adding a command execution endpoint. Focused coverage is in `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked` and `cargo test -p kiana-entrypoints direct_connect_app_contract_schemas_match_packaged_schema_files --locked`.

### Phase 8: Advanced Agent And Knowledge Features

References: AutoGen, MetaGPT, LangChain, OpenHands.

Current verified slice (2026-07-11): `kiana eval run --suite <file> [--baseline <file>] [--json] [--fail-on-failure]` now provides a deterministic offline RuntimeEvent replay and local baseline-regression harness. It performs no model, shell, or network execution; enforces strict `kiana.eval-suite.v1`, `kiana.eval-baseline.v1`, and `kiana.eval-report.v1` contracts; protects suite-relative fixture containment; bounds fixtures; records stable event/tool/error/token/final-result metrics; emits typed findings; and binds suite, fixture, and baseline SHA-256 values. Baseline comparison supports per-case event/tool/token ceilings plus required final status and stop reason; any regression fails the report and `--fail-on-failure`. Usage events require complete input/output token counts encoded as non-negative integers, while malformed or incomplete token fields fail with case/line/field evidence instead of silently becoming zero. The basic suite and baseline ship in release archives and are exercised by real-binary CLI, release/installed-binary smoke, package lifecycle replay, schema smoke, and static preflight. This closes the local protocol-regression and baseline-comparison slice only: fake-provider agent scenarios, external benchmarks, model-quality claims, and customer acceptance remain open Phase 8 work.

Current verified core-runtime slice (2026-07-11): WorkflowRun now has a legal execution path from persisted DAG state to a completed run. `tasks workflow advance` resolves a unique current-node edge, binds real run-local evidence and writes one integrity-contiguous three-event transition batch; `tasks workflow complete` consumes and revalidates a same-run pass VerificationPacket, records its SHA-256/check counts, and closes the run idempotently. This removes the local `workflow.recovery-integrity` blocker caused by the absence of a supported completion path once a real signed release WorkflowRun is generated. It does not remove the separate clean tracked-tree gate or any external release-owner evidence.

Current verified release-proof slice (2026-07-11): `kiana release workflow-proof --json --run-id <run_id> [--out <file>]` now wraps `tasks workflow integrity verify --json`, writes the verified proof to the commercial recovery-integrity proof path, emits a typed `kiana.release-workflow-proof.v1` staging result, and surfaces the exact `KIANA_RELEASE_WORKFLOW_RUN_ID` / `KIANA_WORKFLOW_RECOVERY_INTEGRITY_PROOF_FILE` bindings required by `scripts/commercial-release-blockers-report.sh`. Package lifecycle smoke exercises the command against a completed installed-binary WorkflowRun, schema smoke validates the staging result contract, and release preflight pins the command wiring. This reduces manual proof-staging risk for the local `workflow.recovery-integrity` blocker; it still requires a completed release WorkflowRun from the actual reviewed release state, and it does not remove clean tracked-tree, remote, tag, signing, live-service, or acceptance-owner gates.

Current verified worker-safety slice (2026-07-11): Bounded Swarm worker state now persists `kiana.swarm-worker-state.v2` with nested `kiana.swarm-process-identity.v1`, binding Linux PID, PGID, and `/proc/<pid>/stat` start-time while retaining command SHA-256 as provenance only. `status` and `monitor` fail closed on missing or mismatched identity, and `cancel` uses a two-phase preflight before TERM/KILL so suspicious workers cannot cause partial cancellation or foreign-process termination. `status` and `monitor` also emit top-level `kiana.swarm-process-identity-backend.v1`: Linux reports `backend=linux_procfs` with start/monitor/cancel safety flags, while non-Linux reports `backend=unsupported_platform`, `supported=false`, and `process_identity_backend_not_implemented` instead of implying unimplemented platform safety. `monitor` supports bounded automatic retry for clean `worker_failed` exits only: it archives failed attempt outputs, increments `KIANA_SWARM_ATTEMPT`, and withholds the final ResultPacket until success or retry exhaustion. `status` and `monitor` attach `kiana.swarm-worker-health.v1` to each worker row so downstream UI/CI/orchestrators can read normalized health state, reason, next action, identity, retry, budget, and scope counters without log scraping. Terminal ResultPackets now embed `kiana.swarm-worker-telemetry.v1` with telemetry presence, budgeted command count, parsed commands, invalid command entries, raw telemetry SHA-256, and parser notes while preserving legacy `commands_run`. Focused coverage lives in `kiana-commands/src/swarm_process_identity.rs`, `kiana-commands/tests/swarm_command.rs`, `docs/schemas/kiana-swarm-process-identity-backend.v1.schema.json`, `docs/schemas/kiana-swarm-worker-telemetry.v1.schema.json`, `docs/schemas/kiana-swarm-worker-state.v2.schema.json`, and `bash scripts/schema-contract-smoke.sh`. Remaining Phase 8 worker work is real Windows/macOS process identity backend implementation and external worker telemetry aggregation, not merely documenting the cross-platform boundary.

Current verified memory slice (2026-07-11): Local Structured Memory v1 keeps the legacy `memory.md` surface while adding append-only `$KIANA_HOME/memory/events.jsonl` records with `kiana.memory-record.v1`. `memory append` now redacts obvious secret shapes before persistence, including `token=`, `password=`, `secret=`, `api_key=`, `sk-`, `xoxb-`, and `ghp_`, and records `redaction_count` in both structured records and status/search projections. `memory status --json` emits `kiana.memory-status.v1`, `memory search --json <query>` emits `kiana.memory-search.v1`, and corrupt JSONL fails closed with line-number evidence. This gives the Project OS a local, deterministic memory substrate before MemPalace/claude-memory-style mining, embedding, MCP, and dashboard layers. Focused coverage lives in `kiana-commands/src/memory.rs`, `docs/schemas/kiana-memory-*.v1.schema.json`, `cargo test -p kiana-commands memory --locked --offline --no-fail-fast`, `bash scripts/schema-contract-smoke.sh`, and `KIANA_PREFLIGHT_SKIP_COMPLIANCE=1 bash scripts/release-preflight.sh --local-rc`. Remaining memory work includes automatic session mining, stale-memory invalidation, semantic/vector retrieval, complete PII classification, and multi-user permission boundaries.
...
- Local artifact inventories are exposed through `kiana.context-artifacts.v1` with optional persistent cache delta reporting, context-pack snippet matches are traceable through `kiana.context-artifact-graph.v1`, deterministic vector-search results are exposed through `kiana.context-vector-search.v1`, file-level source/test plus repo-local path-reference relationships are exposed through `kiana.context-artifact-dependency-graph.v1`, `kiana.context-artifact-store.v1` bundles the current inventory, source/test/design/PRD/task role summaries, plus dependency graph into a deterministic manifest with optional persistent cache delta reporting, and `kiana.context-artifact-readiness.v1` reports whether required PRD/design/tasks/source/test artifacts are present before later team-runtime execution; broader durable artifact semantics beyond manifest persistence still belongs to the later team-runtime work.

Progress:

- 2026-07-08: Added `kiana.team-plan.v1` as the first deterministic role-runtime preflight. `kiana tasks plan --json` and `/app/team/plan` now report PM, Architect, Engineer, QA, and Data Analyst task ownership, blocked task counts, dependency edges, and PRD/design/tasks/source/test artifact coverage before fake-model team execution is promoted. Focused coverage is in `cargo test -p kiana-commands team_plan --locked --offline`, `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked --offline`, and `bash scripts/schema-contract-smoke.sh`.
- 2026-07-08: Added a schema-gated team/task status contract for the local knowledge-agent foundation. `kiana tasks json` now emits `kiana.tasks.v1` with task-list identity, task directory, status counts, and ordered tasks, while `/app/team/status` exposes pinned `kiana.app-server.team-status.v1` so Web/IDE clients can inspect durable local team/task state without shelling out. Focused coverage is in `cargo test -p kiana-commands tasks_json_reports_schema_status_counts_and_task_order --locked --offline`, `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked`, and `bash scripts/schema-contract-smoke.sh`.
...
- 2026-07-05: Added deterministic artifact role summaries to `kiana.context-artifact-store.v1`. Artifact-store manifests now count source, test, design, PRD, task, and generic artifacts so future MetaGPT/AutoGen-style team runtime can reason about product/design/task/code/test coverage without embeddings or external services. Focused coverage is in `cargo test -p kiana-query context_artifact_store_reports_artifact_role_summary --locked`, `cargo test -p kiana-commands context_artifact_store_json_reports_manifest --locked`, and `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked`.
- 2026-07-05: Added deterministic artifact readiness reporting. `kiana context artifact-readiness --json` and `/app/context/artifact-readiness` now emit `kiana.context-artifact-readiness.v1` with fixed required-role coverage for PRD, design, tasks, source, and test artifacts, giving future MetaGPT/AutoGen-style role orchestration a local preflight surface before assigning work. Focused coverage is in `cargo test -p kiana-query context_artifact_readiness_reports_missing_required_roles --locked`, `cargo test -p kiana-commands context_artifact_readiness_json_reports_missing_roles --locked`, and `cargo test -p kiana-entrypoints direct_connect_app_contract_exposes_product_shell_endpoints --locked`.
- 2026-07-07: Added the first deterministic vector-search contract. `kiana context vector-search --json` now emits pinned `kiana.context-vector-search.v1` results from an offline `kiana.deterministic-hash-embedding.v1` model with fixed dimensions, content hashes, cosine scores, token-overlap counts, and first-line excerpts; `/app/context/vector-search`, release smoke, package lifecycle smoke, and preflight schema checks cover the surface before production embedding backends or vector stores are wired. Focused coverage is in `cargo test -p kiana-query context_vector_search --locked --offline`, `cargo test -p kiana-commands context_vector_search --locked --offline`, and `cargo test -p kiana-entrypoints direct_connect_app_contract --locked`.
...
# Phase 8 EDA Review Workbench Slice (2026-07-11)

- 已完成 `/eda review` P0 本地审查入口：requirements/schematic/BOM/Gerber/CPL/constraints intake、WorkflowRun/Evidence 绑定、确定性报告和 bring-up 计划。
- 已完成 EDA P1 KiCad XML netlist 结构审查：同一哈希快照解析、组件/BOM 双向覆盖、power/interface/dangling 规则、确定性 summary/check，以及 malformed/unknown-reference fail-closed。
- 已完成可交付门禁：兼容 rules v1/v2 的严格 schema、聚焦命令与 parser 测试、发布包 smoke、release/installed binary smoke。
- 后续 P1/P2：增加 EasyEDA 或其他结构化导出适配器、扩展规则包和 provenance 供应链数据源；完整 ERC、替代料、费用和生产动作继续经过专业工具或 Approval Gate。
