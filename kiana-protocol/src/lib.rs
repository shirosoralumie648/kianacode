...
pub use kiana_domain::{
    ...
    CleanupResourceStatus, ClosingReceipt, CommunicationLifecycleEvent,
    ...
    ConfigSnapshot, ContextCandidate, ContextMaterialClass, ContextMaterialType, ContextPlan,
    ContextPlanItem, ContextSourceBudget, CredentialDisplayStatus, Criterion, CriterionId,
    DataBoundary, DataBoundaryId, DecisionActorKind, DecisionOption, DecisionPurpose, DelegationId,
    DelegationPacket, DeliveryAttempt, DeliveryAttemptId, DeliveryAttemptStatus, DeliveryReceipt,
    DeliveryReceiptId, DeliveryReceiptStatus, DepartmentCatalog, DepartmentSnapshot,
    DispatchIntent, DispatchIntentId, DispatchIntentStatus, DispatchPermit, EffectObservation,
    EffectObservationState, EmbeddingMetadata, EntryPointKind, EntryPointParitySnapshot, EvalCase,
    EvalCaseId, EvalDataset, EvalDatasetId, EvalSplit, EvalSuite, EvalSuiteId, EvalSuiteStatus,
    EventKindSpec, EventStoreHealth, EvidenceId, EvidenceRef, EvidenceRefId, EvidenceStatus,
    ExecutionId, ExecutionReceipt, ExecutionScope, ExecutionStatus, ExtensionCatalog,
    ...
    ExtensionSignature, ExtensionSnapshot, ExtensionSnapshotState, ExtensionType, FenceTokenId,
    Freshness, GoldenTrace, GoldenTraceId, GrantId, HarnessContextStrategy, HarnessEvalComparison,
    HarnessEvalMetric, HarnessEvalMetricObservation, HarnessEvalMetricSet, HarnessEvalMetricStatus,
    HarnessEvalVariant, HarnessExecutionMode, HarnessReplayDifference, HarnessReplayDifferenceKind,
    HarnessReplayMode, HarnessReplayReport, HarnessTraceBinding, HookBudget, HookDecision,
    HookDecisionKind, HookDescriptor, HookExecutionRole, HookFailurePolicy, HookLifecycleBinding,
    HookLifecycleInput, HookLifecyclePlan, HookLifecyclePoint, HookLifecycleResult,
    HookOrderCandidate, HookPhase, HookRunId, HumanDecision, HumanTask, HumanTaskStatus,
    IdentityMigration, IndexCacheKey, IndexComponentKind, IndexGenerationState,
    IndexGenerationStatus, IndexInvalidationPlan, IndexManifest, InteractionId, InvocationId,
    InvocationIdentity, LegacyCheckpointDecision, LegacyCompatibilityDecision, LegacyDisposition,
    LegacyWireKind, LlmTextMetadata, Membership, MembershipId, MembershipStatus, MemoryScope,
    ...
    NormalizedText, Notification, NotificationChannel, NotificationEventClass,
    NotificationEventSource, NotificationEventSpec, NotificationId, NotificationStatus,
    OperationId, OrganizationBinding, OrganizationId, OutputContract, OutputValueType, Partition,
    PartitionId, PartitionProjection, PartitionStatus, PermissionProfile, PluginLifecycle,
    PluginLifecycleState, PolicyProfile, PolicyProfileId, PolicyProfileStatus, Principal,
    PrincipalId, PrincipalKind, PrincipalStatus, ProgressAction, ProgressDecision,
    ProgressEvidence, ProgressInput, ProgressTracker, ProjectAssignment, ProjectAssignmentId,
    ProjectBinding, ProjectId, ProjectIdentity, ProjectTrustSnapshot, ProjectionCheckpoint,
    ProviderAccount, ProviderAccountId, ProviderAccountStatus, ProviderConfigSnapshot,
    ProviderConfigSource, ProviderContinuation, ProviderProfileSnapshot, ProviderSelectionMode,
    Purpose, QualityArtifact, QualityArtifactId, QualityArtifactStatus, QualityStateTransition,
    ...
    RepoMapTaskSelection, RequestId, ResolvedAssignment, ResourceCleanupPlan,
    ResourceCleanupReport, ResourceLease, ResourceRetention, RetrievalCandidate, RetrievalEvidence,
    RetrievalHealth, RetrievalHealthStatus, RetrievalHit, RetrievalItem, RetrievalPack,
    RetrievalProfile, RetrievalRequest, RetrievalResponse, RetrievalResult, RetrievalSourceKind,
    ReviewPacket, RiskLevel, RoleAssignment, RoleAssignmentStatus, RoleCatalog, RoleDescriptor,
    RoleSpec, RunCancellationFact, RunCancellationState, RunId, RunReceipt, RuntimeEvent,
    ...
    SpawnPlanId, StepId, StepIdentity, StorageBackend, StorageCapabilities, StorageError,
    StorageErrorClass, StorageErrorId, StorageHealth, StorageHealthId, StorageHealthStatus,
    StorageIntegrityIncident, StorageIntegrityIncidentClass, StorageIntegrityIncidentId,
    StorageLockId, StorageLockRecord, StorageNamespace, StorageOwnerScope, StorageRetryDisposition,
    StorageRoot, StorageRootId, StorageSchemaRegistry, StoreIdentity, StoreIdentityId,
    Subscription, SubscriptionId, SubscriptionStatus, SupervisionLease, SupervisionLeaseId,
    SwarmGraphError, SwarmLineage, SwarmPlanId, SwarmTransitionEntity, SwarmTransitionEvent,
    SwarmTransitionReducer, SwarmWorkGraph, Symposium, TemplateId, TextNormalizationProfile,
    ToolSpec, TurnId, TurnIdentity, TurnOutcome, TurnOutcomeInput, TurnOutcomeKind, TurnSemantics,
    WorkPacket, WorkPacketStatus, WorkspaceBinding, WorkspaceChange, WorkspaceChangeKind,
    WorkspaceEntryKind, WorkspaceFileIdentity, WorkspaceFileSnapshot, WorkspaceId,
    WorkspaceReadDisposition, WorkspaceSnapshot, WorkspaceSnapshotLimits, WorkspaceTrust,
    ACTION_REF_SCHEMA, ADAPTER_RESULT_MAX_EVIDENCE, ADAPTER_RESULT_MAX_OUTPUT_BYTES,
    ADAPTER_RESULT_SCHEMA, ADAPTER_RESULT_VERSION, APPROVAL_CONSUMPTION_FACT_SCHEMA,
    APPROVAL_DECISION_FACT_SCHEMA, APPROVAL_FACT_VERSION, APPROVAL_MATERIAL_SCHEMA,
    APPROVAL_MATERIAL_VERSION, APPROVAL_PLAN_PREVIEW_SCHEMA, APPROVAL_PLAN_PREVIEW_VERSION,
    ARTIFACT_REF_SCHEMA, ARTIFACT_VERSION_SCHEMA, AUTHENTICATED_PRINCIPAL_SCHEMA,
    AUTHORITY_FENCE_SCHEMA, AUTHORITY_FENCE_VERSION, AUTHORITY_LEDGER_SCHEMA,
    AUTHORITY_SNAPSHOT_SCHEMA, BUDGET_FACT_VERSION, BUDGET_RESERVATION_SCHEMA,
    BUDGET_SETTLEMENT_SCHEMA, CAPABILITY_OUTCOME_SCHEMA, CAPABILITY_RESULT_DIMENSIONS_SCHEMA,
    CAPABILITY_RESULT_RECEIPT_SCHEMA, CAPABILITY_RESULT_RECEIPT_VERSION,
    CHILD_HARNESS_CANCEL_SCHEMA, CHILD_HARNESS_INTENT_SCHEMA, CHILD_HARNESS_OUTCOME_SCHEMA,
    CHILD_HARNESS_VERSION, CHUNK_RANGE_SCHEMA, CHUNK_SCHEMA_VERSION, CHUNK_SET_SCHEMA,
    CLARIFICATION_ANSWER_SCHEMA, CLARIFICATION_REQUEST_SCHEMA, CLARIFICATION_RESOLUTION_SCHEMA,
    CLARIFICATION_VERSION, CLARIFICATION_WAITING_STATUS, CLARIFICATION_WAIT_SCHEMA,
    COMMUNICATION_LIFECYCLE_SCHEMA, COMMUNICATION_MESSAGE_SCHEMA, COMPANY_COMMAND_POLICY_SCHEMA,
    COMPANY_COMMAND_RECEIPT_SCHEMA, COMPANY_DISPATCH_INTENT_SCHEMA, CONFIG_SNAPSHOT_SCHEMA,
    CONTEXT_PLAN_SCHEMA, DATA_BOUNDARY_SCHEMA, DELIVERY_ATTEMPT_SCHEMA, DELIVERY_RECEIPT_SCHEMA,
    ...
    EMBEDDING_METADATA_SCHEMA, ENTRYPOINT_PARITY_SCHEMA, ENTRYPOINT_PARITY_SCHEMA_VERSION,
    ...
    EXECUTION_IDENTITY_SCHEMA_VERSION, EXECUTION_RECEIPT_SCHEMA, EXECUTION_SCOPE_SCHEMA,
    ...
    EXTENSION_SNAPSHOT_SCHEMA, EXTENSION_SOURCE_RESOLUTION_SCHEMA, GOLDEN_TRACE_SCHEMA,
    HARNESS_EVAL_COMPARISON_SCHEMA, HARNESS_EVAL_SCHEMA_VERSION, HARNESS_METRIC_SET_SCHEMA,
    HARNESS_REPLAY_REPORT_SCHEMA, HARNESS_TRACE_BINDING_SCHEMA, HOOK_DECISION_SCHEMA,
    HOOK_DESCRIPTOR_SCHEMA, HOOK_LIFECYCLE_BINDING_SCHEMA, HOOK_LIFECYCLE_INPUT_SCHEMA,
    HOOK_LIFECYCLE_PLAN_SCHEMA, HOOK_LIFECYCLE_RESULT_SCHEMA, HOOK_LIFECYCLE_VERSION,
    HOOK_MANIFEST_SCHEMA, HUMAN_DECISION_SCHEMA, HUMAN_TASK_SCHEMA, IDENTITY_MIGRATION_SCHEMA,
    INDEX_CACHE_KEY_SCHEMA, INDEX_GENERATION_STATE_SCHEMA, INDEX_GENERATION_VERSION,
    INDEX_INVALIDATION_SCHEMA, INDEX_INVALIDATION_VERSION, INDEX_MANIFEST_SCHEMA,
    INVOCATION_IDENTITY_SCHEMA, LEGACY_CHECKPOINT_DECISION_SCHEMA, LEGACY_COMPATIBILITY_SCHEMA,
    LEGACY_COMPATIBILITY_VERSION, LLM_TEXT_METADATA_SCHEMA, MEMBERSHIP_SCHEMA, MEMORY_SCOPE_SCHEMA,
    MERGE_RECEIPT_PATH, MESSAGE_SCHEMA, MODEL_ATTEMPT_IDENTITY_SCHEMA, MODEL_CONTENT_SCHEMA,
    MODEL_OUTCOME_SCHEMA, NORMALIZED_TEXT_SCHEMA, NOTIFICATION_EVENT_REGISTRY_SCHEMA,
    NOTIFICATION_SCHEMA, OUTPUT_CONTRACT_SCHEMA, PLUGIN_LIFECYCLE_SCHEMA, PLUGIN_MANIFEST_SCHEMA,
    POLICY_PROFILE_SCHEMA, PRINCIPAL_SCHEMA, PROGRESS_DECISION_SCHEMA, PROGRESS_EVIDENCE_SCHEMA,
    PROGRESS_TRACKER_SCHEMA, PROJECTION_CHECKPOINT_SCHEMA, PROJECTION_CHECKPOINT_VERSION,
    PROJECT_ASSIGNMENT_SCHEMA, PROJECT_IDENTITY_SCHEMA, PROJECT_TRUST_SNAPSHOT_SCHEMA,
    PROVIDER_ACCOUNT_SCHEMA, PROVIDER_CONFIG_SNAPSHOT_SCHEMA, PROVIDER_CONTINUATION_SCHEMA,
    PROVIDER_PROFILE_SNAPSHOT_SCHEMA, QUALITY_ARTIFACT_SCHEMA, QUALITY_TRANSITION_SCHEMA,
    ...
    RESOLVED_ASSIGNMENT_SCHEMA, RESOLVED_STEP_CONTEXT_SCHEMA, RESOURCE_CLEANUP_PLAN_SCHEMA,
    RESOURCE_CLEANUP_REPORT_SCHEMA, RESOURCE_CLEANUP_VERSION, RESOURCE_LEASE_SCHEMA,
    RESOURCE_LEASE_VERSION, RESOURCE_RETENTION_SCHEMA, RETRIEVAL_CANDIDATE_SCHEMA,
    RETRIEVAL_EVIDENCE_SCHEMA, RETRIEVAL_HEALTH_SCHEMA, RETRIEVAL_PACK_SCHEMA,
    RETRIEVAL_REQUEST_SCHEMA, RETRIEVAL_RESPONSE_SCHEMA, RETRIEVAL_RESULT_SCHEMA,
    RETRIEVAL_VERSION, REVIEW_PACKET_SCHEMA, ROLE_ANALYST, ROLE_ARCHITECT, ROLE_ASSIGNMENT_SCHEMA,
    ROLE_BUILDER, ROLE_CATALOG_SCHEMA, ROLE_CLOSER, ROLE_INPUT_SCHEMA_PREFIX, ROLE_LIBRARIAN,
    ROLE_OUTPUT_SCHEMA_PREFIX, ROLE_PM, ROLE_QA, ROLE_REVIEWER, ROLE_SPEC_SCHEMA,
    RUNTIME_EVENT_SCHEMA, RUN_CANCELLATION_SCHEMA, RUN_CANCELLATION_VERSION, RUN_RECEIPT_SCHEMA,
    SCOPE_SET_SCHEMA, SCOPE_SET_SCHEMA_VERSION, SECRET_REF_SCHEMA, SECURITY_OBJECT_SCHEMA,
    SECURITY_OBJECT_VERSION, SECURITY_REASON_SCHEMA, SECURITY_REASON_VERSION,
    SECURITY_SCHEMA_REGISTRY_SCHEMA, SECURITY_SCHEMA_REGISTRY_VERSION, SERVICE_IDENTITY_SCHEMA,
    SESSION_ASSERTION_SCHEMA, SESSION_ASSERTION_VERSION, SESSION_ASSIGNMENT_SCHEMA,
    SHARING_GRANT_SCHEMA, SKILL_DESCRIPTOR_SCHEMA, SOURCE_CHUNK_SCHEMA, SOURCE_REF_SCHEMA,
    SOURCE_SNAPSHOT_SCHEMA, STEP_IDENTITY_SCHEMA, STORAGE_CAPABILITIES_SCHEMA,
    STORAGE_ERROR_SCHEMA, STORAGE_HEALTH_SCHEMA, STORAGE_INTEGRITY_INCIDENT_SCHEMA,
    STORAGE_LOCK_SCHEMA, STORAGE_OWNER_SCOPE_SCHEMA, STORAGE_ROOT_SCHEMA,
    STORAGE_SCHEMA_REGISTRY_SCHEMA, STORE_IDENTITY_SCHEMA, SUBSCRIPTION_SCHEMA,
    SWARM_LINEAGE_SCHEMA, SWARM_PARTITION_SCHEMA, SWARM_TRANSITION_EVENT_SCHEMA,
    SWARM_WORK_GRAPH_SCHEMA, TEXT_NORMALIZATION_PROFILE_SCHEMA, TEXT_NORMALIZATION_VERSION,
    TOOL_AUTHORITY_SCHEMA, TOOL_SPECS, TRUST_SNAPSHOT_VERSION, TURN_IDENTITY_SCHEMA,
    TURN_OUTCOME_SCHEMA, TURN_OUTCOME_VERSION, UNIFIED_RETRIEVAL_PROFILE_SCHEMA,
    UNIFIED_RETRIEVAL_VERSION, WORKSPACE_FILE_SNAPSHOT_SCHEMA, WORKSPACE_SNAPSHOT_SCHEMA,
