...
**Created:** 2026-07-26 | **Updated:** 2026-08-22 | **父需求：** DIF-12 | **主旅程：** core.release-lifecycle
...
- 当前基线: local_behavior landed (`rust-toolchain.toml` + upgrade policy + original CI `@master`). Later `release-tui.yml` `@stable` regression was corrected 2026-08-22. Target CI run evidence is still required.
- 设计引用: DESIGN-INDEX Phase 2 行；02-CONTEXT.md canonical_refs
- 依赖: None
- 状态: local_implemented; verification_open
...
- 当前基线: local_behavior schema + CI record step exist. `target_environment` still requires a real CI-produced `dist/build-inputs.json`. A 2026-08-22 verification fix made the record step export live rustc/lock/HEAD values instead of empty GitHub `env.*` mappings.
- 设计引用: 02-CONTEXT.md specifics
- 依赖: FEAT-02-01
- 状态: local_implemented; target_environment_open
...
- 当前基线: local_behavior — `--md` / `--audience=user` landed in 0eaaa9e; missing generated artifacts still render as blocking / 需要操作
...
- 当前基线: not in Phase 2 closeout — Wave 3 only added the SBOM signing hook. Full `dist/proofs/signing/release-signature.json` / Sigstore remains a later/external blocker (D-11, Phase 24 family).
- 设计引用: DESIGN-INDEX T2；02-CONTEXT.md specifics（ruflo 签名参考）
- 依赖: None
- 状态: deferred_external; not_phase2_complete
...
- Verifier: 签名验证 + 脱敏断言
- 当前基线: local_behavior script landed (`scripts/sign-sbom.sh` redacted user SBOM + optional `KIANA_SIGNING_COMMAND`). `dist/sbom.cdx.json` / `.sig` remain generated artifacts and stay blocking until a real build/signing run.
- 设计引用: DESIGN-INDEX T2 compliance
- 依赖: FEAT-02-04
- 状态: local_implemented; signing_open
...
- 当前基线: local_behavior script/schema landed (`scripts/generate-license-summary.sh`, `kiana.license-compliance-summary.v1`). `dist/license-summary.json` stays blocking until the script is run in the build flow.
- 设计引用: DESIGN-INDEX T2
- 依赖: None
- 状态: local_implemented; dist_artifact_open
...
- 当前基线: local_behavior — blocker report already exposes acceptance/platform/signing holes as blocking / 需要操作; Phase 2 does not collect target-environment or user-value evidence.
- 设计引用: DESIGN-INDEX T2 proof-templates
- 依赖: FEAT-02-03
- 状态: local_implemented; evidence_collection_deferred
