...
北星是 Company OS（见 `COMPANY.md`）。v0.4 Phase 1（`REV-01`）、Phase 2（`CODE-01` 矩阵草稿）与 Phase 3（`CODE-02` stdio MCP client）已本地绿。下一执行集是 **v0.4.3 skills/hooks 接到 harness**（`CODE-03`）。五部门与六层 RAG 仍冻结到 v0.5。
